...
export function convertWhaSpellMakerDocument(input, options = {}) {
  ...
  const custom = collectCustomImages(customImages, warnings);
  ...
  input.seals.forEach((sealValue, sealIndex) => {
    ...
    rings.forEach((ringValue, index) => {
      ...
      const openingAngle = (
        270 + transform.angleDegrees + number(ringValue.openingAngle, 0, "ring opening angle")
      ) % 360;
      ...
      addAction(actions, {
        ...
        radius,
        width,
        ...colorField(ringValue.color ?? DEFAULT_INK, "ring color"),
        closed: openingSize <= 0,
        ...(ringValue.filled === true ? { filled: true } : {}),
        ...renamedColorField(ringValue.fillColor, "fillColor", "ring fill color"),
    ...
    sigils.forEach((sigilValue, index) => {
      const action = convertGlyph(sigilValue, "sigil", transform, warnings, index, custom.bySymbolName);
      if (!action) return;
    ...
    signs.forEach((signValue, index) => {
      const converted = convertSign(signValue, index, transform, warnings, custom.bySymbolName);
      if (!converted) return;
  ...
  const circle = parseCircleShare({
    version: 1,
    locale,
    title,
    canvas: CANVAS,
    ...(custom.assets.length > 0 ? { assets: custom.assets } : {}),
    actions,
  }, {
...
function convertGlyph(value, requestedKind, transform, warnings, index, customImages) {
  ...
  if (customImage) {
    return {
      ...
      ...(value.tinted === true ? { tinted: true, ...colorField(value.color ?? "#000000", `${requestedKind} color`) } : {}),
    };
  }
  return {
    type: "glyph",
    ...
    size,
    rotation: transform.angle + degrees(angle),
    ...colorField(value.tinted === true ? value.color ?? DEFAULT_INK : DEFAULT_INK, `${requestedKind} color`),
    ...(value.tinted === true ? { tinted: true } : {}),
  };
}

function convertSign(value, index, transform, warnings, customImages) {
  ...
  if (entry && entry.kind !== "sign") warnOnce(warnings, `The sign "${String(value.name)}" was mapped to the catalogue kind "${entry.kind}".`);
  ...
  for (let copy = 0; copy < copyCount; copy += 1) {
    ...
    actions.push(customImage ? {
      ...
      ...(value.tinted === true ? { tinted: true, ...colorField(value.color ?? "#000000", "sign color") } : {}),
    } : {
      type: "glyph",
      element: entry.name,
      kind: "sign",
      ...
      size,
      rotation: transform.angle + theta + angle,
      ...colorField(value.tinted === true ? value.color ?? DEFAULT_INK : DEFAULT_INK, "sign color"),
      ...(value.tinted === true ? { tinted: true } : {}),
    });
  }
  return actions;
...
function convertLine(value, transform, warnings, index) {
  ...
  return {
    type: "free",
    width: scaledNumber(value.weight ?? value.lineWeight, 10, transform.scale, "line weight"),
    points,
    ...colorField(value.color ?? DEFAULT_INK, "line color"),
  };
}

function collectCustomImages(value, warnings) {
  if (value === undefined) return { assets: [], bySymbolName: new Map() };
  if (!Array.isArray(value)) throw new TypeError("WHA custom images must be an array");
  const assets = [];
  const bySymbolName = new Map();
  value.forEach((item, index) => {
    if (!isRecord(item) || typeof item.name !== "string" || !item.name.trim()) {
      warnOnce(warnings, "A custom image with an invalid name was skipped.");
      return;
    }
    if (!isEmbeddedRaster(item.file)) {
      warnOnce(warnings, "A custom image was skipped because only embedded PNG, JPEG, or WebP data is supported.");
      return;
    }
    const asset = {
      id: `wha-custom-${index + 1}`,
      src: item.file,
      name: item.name.trim().slice(0, 80),
    };
    assets.push({ id: asset.id, src: asset.src });
    bySymbolName.set(`custom_${item.name}`, asset);
  });
  return { assets, bySymbolName };
}

function resolveCustomImage(rawName, customImages) {
  return typeof rawName === "string" ? customImages.get(rawName) ?? null : null;
}

function isEmbeddedRaster(value) {
  return typeof value === "string" && /^data:image\/(png|jpeg|webp);base64,[a-z0-9+/]+={0,2}$/i.test(value);
}

function colorField(value, label) {
  if (value === undefined || value === null || value === "") return {};
  if (typeof value !== "string" || !/^#[0-9a-f]{6}$/i.test(value)) throw new TypeError(`${label} must be a six-digit hexadecimal color`);
  return { color: value };
}

function renamedColorField(value, key, label) {
  const parsed = colorField(value, label);
  return parsed.color ? { [key]: parsed.color } : {};
}
...
function createTransform(seal) {
  ...
  const angleDegrees = number(seal.angle, 0, "seal angle");
  const angle = degrees(angleDegrees);
